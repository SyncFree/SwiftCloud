...
public class SetTxnLocalOrder extends BaseCRDTTxnLocal<SetOrder> {
	private Map<OrderLine, Set<TripleTimestamp>> elems;

	public SetTxnLocalOrder(CRDTIdentifier id, TxnHandle txn,
			CausalityClock clock, SetOrder creationState,
			Map<OrderLine, Set<TripleTimestamp>> elems) {
	 ...
	 * @param e
	 */
	public void insert(OrderLine e) {
		...
		adds.add(ts);
		registerLocalOperation(new SetInsert<OrderLine, SetOrder>(ts, e));
	}
	 ...
	 * @param e
	 */
	public void remove(OrderLine e) {
		Set<TripleTimestamp> ids = elems.remove(e);
		if (ids != null) {
			TripleTimestamp ts = nextTimestamp();
			registerLocalOperation(new SetRemove<OrderLine, SetOrder>(ts, e,
					ids));
		}
	}
	...
	public Set<OrderLine> getValue() {
		return elems.keySet();
	}

	@Override
	public Object executeQuery(CRDTQuery<SetOrder> query) {
