...
#	run_cmd $client2 "mkdir $modeDir"
	for ratio_i in $ratios; do
		for workers in $numWorkers; do
			...
			run_cmd_bg $client4 $CMD $sampleSize $cltSize $maxTxSize $workers $ratio_i $executionTime $numRuns "${modes[$i]}" $server2 $rawDir

			echo "starting client 5"
			run_cmd $client5 $CMD $sampleSize $cltSize $maxTxSize $workers $ratio_i $executionTime $numRuns "${modes[$i]}" $server2 $rawDir
