...
echo $runDir
mkdir -p $runDir
output_prefix=$runDir/$SCENARIO-$DC_NUMBER-$SCOUTS_NUMBER-$CLIENTS_NUMBER-$INPUT_USERS-$INPUT_ACTIVE_USERS-$ISOLATION-$CACHING-$NOTIFICATIONS-$CACHE_EVICTION_TIME_MS-$ASYNC_COMMIT-$THINK_TIME_MS-$DURATION.log
