...
class CRDTData
{
    ...
    CRDTData( CRDT<?> crdt, CausalityClock clock, CausalityClock pruneClock) {
        this.crdt = crdt;
        this.clock = clock;
...
class DCSurrogate extends Handler implements swift.client.proto.SwiftServer {
     ...
     */
    void putCRDT( CRDTIdentifier id, CRDT<?> crdt, CausalityClock clk, CausalityClock prune) {
        ...
        synchronized( m) {
            CRDTData data = m.get( id.getKey());
            if( data == null) {
                data = new CRDTData( crdt, clk, prune);
    ...
    boolean execCRDT( CRDTObjectOperationsGroup grp) {
        CRDTIdentifier id = grp.getTargetUID();
        CRDTData data = getCRDT( id, null);
        if( data == null)
            return false;
        Iterator<CRDTOperation> it = grp.operations.iterator();
        while( it.hasNext()) {
            CRDTOperation o = it.next();
    ...
    public void onReceive(RpcConnection conn, FetchObjectVersionRequest request) {
        ...
        if( crdt == null) {
            conn.reply( new FetchObjectVersionReply( FetchObjectVersionReply.FetchStatus.OBJECT_NOT_FOUND, 
                    null, request.getVersion(), ClockFactory.newClock()));
        } else {
            synchronized( crdt) {
                conn.reply( new FetchObjectVersionReply( FetchObjectVersionReply.FetchStatus.OK, 
                    crdt.crdt, crdt.clock, crdt.pruneClock));
            }
        }
    }
    ...
    public void onReceive( final RpcConnection conn, GenerateTimestampRequest request) {
        ...
        sequencerClientEndpoint.send( sequencerServerEndpoint, request,
