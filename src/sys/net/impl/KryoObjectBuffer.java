...
public class KryoObjectBuffer {
	...
	/**
	 * Creates an ObjectStream with an initial buffer size of 2KB and a maximum
	 * size of 16KB.
	 */
	public KryoObjectBuffer(Kryo kryo) {
		this(kryo, 2 * 1024, 16 * 1024);
	}

	/**
	 * @param maxCapacity
	 *            The initial and maximum size in bytes of an object that can be
	 *            read or written.
	 * @see #ObjectBuffer(Kryo, int, int)
	 */
	public KryoObjectBuffer(Kryo kryo, int maxCapacity) {
		this(kryo, maxCapacity, maxCapacity);
	}

	/**
	 * @param initialCapacity
	 *            The initial maximum size in bytes of an object that can be
	 *            read or written.
	 * @param maxCapacity
	 *            The maximum size in bytes of an object that can be read or
	 *            written. The capacity is doubled until the maxCapacity is
	 *            exceeded, then SerializationException is thrown by the read
	 *            and write methods.
	 */
	public KryoObjectBuffer(Kryo kryo, int initialCapacity, int maxCapacity) {
		this.kryo = kryo;
	...
	public <T> T readClassAndObject(DataInputStream input) throws IOException {
		return (T) readClassAndObject(input, input.readInt());
	}
	 ...
	 * @see Kryo#readClassAndObject(ByteBuffer)
	 */
	private Object readClassAndObject(InputStream input, int contentLength) {
	...
	//
	// /**
	// * Reads the specified number of bytes and returns the deserialized
	// object.
	// * @see Kryo#readObject(ByteBuffer, Class)
	// */
	// public <T> T readObject (InputStream input, int contentLength, Class<T>
	// type) {
	// readToBuffer(input, contentLength);
	// return kryo.readObject(buffer, type);
	// }
	...
	// * @see Kryo#readObjectData(ByteBuffer, Class)
	// */
	// public <T> T readObjectData (InputStream input, int contentLength,
	// Class<T> type) {
	// readToBuffer(input, contentLength);
	// return kryo.readObjectData(buffer, type);
	// }
	...
	// throw new
	// SerializationException("Buffer limit exceeded serializing object of type: "
	 ...
	 * @see Kryo#writeClassAndObject(ByteBuffer, Object)
	 */
	public void writeClassAndObject(Object object, DataOutputStream dos) throws IOException {
		buffer.clear();
		while (true) {
			try {
				kryo.writeClassAndObject(buffer, object);
